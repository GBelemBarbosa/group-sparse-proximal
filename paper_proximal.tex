...
that the classical cardinality-constrained formulation cannot express. The main
result is a complete characterization of the proximal mapping of the resulting
discontinuous regularizer, extending Theorem~3.2 of
\citeauthor{beck2018} (\citeyear{beck2018}). In particular, we identify which
groups can be activated, prove that the proximal mapping is empty precisely
when fewer groups can be switched on than the lower bound requires, and give an
explicit description of every proximal point together with an algorithm that
computes one. The proof extends their comparison of groupwise gains to the
case in which the lower bound is active.
\end{abstract}
...
The main result is a characterization of the proximal mapping of this
regularizer, extending \cite[Theorem~3.2]{beck2018} from an upper bound alone
to a window on the number of active groups. With a lower bound the mapping can
be empty, which never happens when only an upper bound is present. That
emptiness is not an obstacle if the group constraints are modelled
appropriately, as discussed in \cref{sec:emptyfix}. When the mapping is
nonempty we describe every proximal point and give an algorithm that computes
one.
...
upper bound \(s\) caps how many sectors may be used; a positive lower bound \(l\)
enforces a minimal diversification
across them, in the same spirit as sparse Markowitz models
\cite{brodie2009}.
\end{example}
...
This same scenario is an example of poor model for a lower bound. If \(D_i\) contains both
the origin and a neighbourhood of it, a method can activate a group with
an arbitrarily small block, counting it towards the quota \(l\) without any
meaningful diversification. A constraint that is meant to be either off or
genuinely on should exclude a neighbourhood of the origin. We return to this
in \cref{sec:emptyfix}.
...
\end{proof}
This emptiness is not an obstacle if the constraints are modelled
appropriately; see \cref{sec:emptyfix}.
...
\section{Appropriate modelling and empty proximal mappings}
\label{sec:emptyfix}

\Cref{alg:proxh} returns \(\emptyset\) when \(l\ge 1\) and
\(|I_{\mathcal{A}}(\x)|<l\). That answer is faithful to \eqref{h}: there is no
feasible point of \(B\cap C_{l,s}\) assembled from groupwise projections. A
proximal-gradient loop, however, cannot take a step from an empty mapping.

The emptiness is a modelling symptom, not a defect of the oracle. It appears
when \(D_i\) contains the origin as an accumulation point, so that a group may
be recorded as active with an arbitrarily small block. The same geometry lets
a method meet the quota \(l\) by allocating a negligible amount on extra groups,
which is not the diversification the lower bound was meant to enforce. If a
group is to count as active, its block should be large enough to matter in the
application.

A direct way to encode that requirement is to cut from each \(D_i\) the open
ball of radius \(\varepsilon_i>0\) about the origin, in the Euclidean or
\(\ell_\infty\) norm,
\[
D_i^{\varepsilon}\coloneqq D_i\setminus\bigl\{\z\in\R^{n_i}:\|\z\|<\varepsilon_i\bigr\}.
\]
The origin remains available through \(B\), so a group can still be switched
off. An active group must leave the ball. Therefore, the radius should be a meaningful
scale of the model rather than a machine epsilon. In the sector portfolio of
\cref{ex:carteira}, a sector that is used at all might be required to receive,
say, five percent of the budget, thus guaranteeing an allocation that represents genuine diversification. 

If \(\forall i\ \varepsilon_i\) is small enough that \(D_i\) is not swallowed by the
ball, then \(D_i^{\varepsilon}\) is closed and does not contain the origin, so
every group is activatable for every \(\x\). The modified
regularizer then has a nonempty proximal mapping everywhere. In that situation the emptiness test in
\Cref{alg:proxh} is redundant and one may skip it and continue from the
computation of \(s'\) onward.

If one keeps \(D_i\) up to the origin, emptiness can still occur as the
coincidence \(P_{D_i}(\x_{G_i})=\{\mathbf{0}\}\) at a single point. A small
perturbation of \(\x_{G_i}\) is then a numerical safeguard, but it does not
replace the modelling choice above.
...
\eqref{P}. Under the modelling of \cref{sec:emptyfix}, a hole about the origin
in each \(D_i\) makes every surviving group activatable, so the emptiness of
\(\prox_h\) need not arise.
